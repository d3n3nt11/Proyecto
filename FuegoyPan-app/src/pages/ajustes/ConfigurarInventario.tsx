...
import { getIngredientes, modificarInventario } from "../../data/api";


export default function ConfigurarInventario() {
  ...
  useEffect(() => {
    const cargarIngredientes = async () => {
      ...
      const porcenatjeInicial: { [key: number]: number } = {};
      data.forEach((ingredient) => {
        porcenatjeInicial[ingredient.ingredientId] = ingredient.minStock;
      });
      setPorcentaje(porcenatjeInicial);
      } catch (error) {
  ...
  const aumentarPorcentaje = (id: number) => {
    const nuevoValor = (porcentaje[id] || 0) + 1;
    setPorcentaje((prev) => ({
      ...prev,
      [id]: nuevoValor,
    }));
    modificarInventario(id, nuevoValor);
  };

  const disminuirPorcentaje = (id: number) => {
    const nuevoValor = (porcentaje[id] || 0) - 1;
          ...
          {filteredIngredients.map((ingredient) => (
            <div key={ingredient.ingredientId} className="bg-white rounded-2xl shadow-lg p-3 flex flex-row items-center justify-between gap-3 mb-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => disminuirPorcentaje(ingredient.ingredientId)}
