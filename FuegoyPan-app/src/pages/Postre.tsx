...
import { getPostres } from "../data/api";
...
export default function Postre() {
    ...
    const [filteredPostres, setFilteredPostres] = useState<IProducto[]>([]);

    // UseEffect para cargar los postres
    useEffect(() => {
    ...
    useEffect(() => {
        if (search.trim() === "") {
            setFilteredPostres(productos);
        } else {
            const findPostres = productos.filter((p) =>
            p.name.toLowerCase().includes(search.toLowerCase())
            );
            setFilteredPostres(findPostres);
        }
    }, [search, productos]);
    ...
    return (
        <div className="bg-[#F2E9DB] min-h-screen flex flex-col items-center py-10">
            ...
            {!loading && filteredPostres.length === 0 && (
